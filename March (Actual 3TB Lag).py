...
df_cli=df_cli.loc['2024-01-01':'2024-03-31']
print(df_cli)

date_range = pd.date_range(start='2024-01-01',end='2024-03-31 23:45:00',freq='15min')

df_cli.set_index(date_range,inplace=True)
df_cli.drop(['block','temp','ws','rh'],axis=1,inplace=True)
print(df_cli)

time_range = pd.date_range(start='2024-03-01 00:00:00',end='2024-03-01 23:45:00',freq='15min').time
date_range = pd.date_range(start='2024-03-01',end='2024-03-31',freq='D')

march_df = pd.DataFrame(index=time_range)
day_start = pd.to_datetime('2024-03-01 00:00:00')
for i in date_range:
...
model_run_time = pd.to_datetime('2024-02-29 22:30:00')
march_forecasted_hour_df = pd.DataFrame(index=time_range)
march_forecasted_day_df = pd.DataFrame(index=time_range)
MAPE_df = pd.DataFrame(index=time_range[::6])
MAE_df = pd.DataFrame(index=time_range[::6])
file_path = 'march3TB.xlsx'
...
for iteration in range(1, 32):
    ...
    while model_run_time != day_end:
        ...
        march_forecasted_hour_df[model_run_time] = np.nan
        march_forecasted_hour_df.loc[test_start_date.time():test_end_date.time(), model_run_time] = forecast_mean[test_start_date:].values
    ...
    march_forecasted_day_df[model_run_time.date()] = day
    MAPE_df[test_end_date.date()] = MAPE_list
    MAE_df[test_end_date.date()] = MAE_list
    print(MAPE_df)

    with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
        march_forecasted_hour_df.to_excel(writer, sheet_name='march_forecast_hour', index=True)
        march_forecasted_day_df.to_excel(writer, sheet_name='march_forecast_day', index=True)
        march_df.to_excel(writer, sheet_name='march_test', index=True)
